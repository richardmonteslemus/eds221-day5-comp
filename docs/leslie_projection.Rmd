...
# The line below time vector/series for each year from a range of 0 - 8 incrementing by 1 

time_yr <- seq(from = 0, to = 8, by = 1)

# The line below creates a new matrix showing the initial population for each time value in time_yr vector. Columns are egss, fry, juv, adult and rows are time. 
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# Populate first row with fish population info at year zero.
proj_year[1, ] <- fish_year0

# The for loop below estimates population size for each year based on population size from previous year. 
...
#updating column names for the population 
